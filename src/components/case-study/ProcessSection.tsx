...
import type { ProjectSection, ProcessBlock } from "@/data/projects";

function renderBlock(block: ProcessBlock, bi: number) {
  if (block.type === "list") {
    return (
            ...
            <ScrollReveal key={section.title} delay={i * 0.08}>
              <div className={useSideLayout ? `flex flex-col gap-8 md:grid md:grid-cols-2 md:gap-12 md:items-center` : ``}>
                <div className={useSideLayout && isReversed ? "md:order-2" : ""}>
                  ...
                  {section.blocksBeforeImage && section.blocks?.map((block, bi) => {
                  ...
                  {!section.blocksBeforeImage && section.blocks?.map((block, bi) => renderBlock(block, bi))}
